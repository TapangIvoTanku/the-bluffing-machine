...
- If Receiver **Attacks**:
    ...
    - If Sender is **Low Resolve**: Sender capitulates. Payoffs: $(-1, 1)$.
...
\begin{figure}[H]
    ...
    \caption{Bluffing Frequency by Model and Treatment}\label{fig:bluffing_freq}
...
Figure \ref{fig:grouped_bar} provides a detailed view of bluffing rates across all six experimental cells, with 95\% Wilson confidence intervals. The grouped bar chart makes the treatment effect immediately visible: GPT-4.1-mini and GPT-4.1-nano show a dramatic swing from 0\% in the zero-shot condition to 100\% and 78\% respectively in the role-conditioned treatment, while Gemini-2.5-Flash bluffs aggressively (93\% and 89\%) in both conditions. All deviations from the PBE benchmark of 42\% are statistically significant at the $p < 0.001$ level (binomial test).
...
To understand the concepts driving strategic choices, we conducted a keyword analysis of all 900 reasoning traces. Figure \ref{fig:heatmap} shows the percentage of traces in each experimental cell that contain keywords associated with seven strategic concepts. The results reveal a striking pattern: models overwhelmingly invoke cost/risk reasoning (up to 99\% of traces) and deterrence language (up to 100\% in role-conditioned cells), but genuine rational calculation is nearly absent (0--2\% across all cells). Explicit deception/bluffing language appears only in role-conditioned treatments, confirming that prompt framing is the primary driver of strategic behavior.
...
\begin{itemize}
    \item \textbf{Signal Justification:} Most justifications are based on perceived capabilities or risk avoidance. Explicit strategic deception is rare (less than 20\% of traces).
    \item \textbf{Deception Awareness:} In the role-conditioned treatment, models show some awareness of deception, but in the zero-shot condition, they are almost entirely honest.
    \item \textbf{Receiver Inference:} Receivers overwhelmingly claim to be performing a Bayesian update (98.7\%), but the calibration curves show this is not true.
    \item \textbf{Belief Attribution:} All models show a high degree of belief attribution, explicitly modeling the opponent\'s beliefs in their reasoning.
\end{itemize}
...
Figure \ref{fig:dashboard} presents a complete summary dashboard of all six key metrics across all experimental conditions. The dashboard confirms the consistency of our findings: no model achieves the PBE bluffing rate of 42\%, all models exhibit high Brier scores indicating poor Bayesian calibration, and the Equilibrium Deviation Index confirms systematic departures from rational play across all conditions.
...
\begin{figure}[H]
    ...
    \caption{Prompt Sensitivity Analysis Results}\label{fig:sensitivity}
...
This study has several limitations. First, our game is a highly stylized representation of a real-world crisis. A critic might rightly question the ecological validity of a simple, binary signaling game. However, this stylization is a deliberate methodological choice. By abstracting away the complexities of real-world crises, the signaling game framework provides a controlled experimental environment that allows us to isolate and test a foundational strategic mechanism: deception. While less realistic on the surface, this approach provides a degree of analytical rigor and causal leverage that is difficult to achieve in more complex, open-ended simulations. Understanding how LLMs behave in this simple, foundational case is a necessary prerequisite for analyzing their behavior in more complex settings. Future research should build on this foundation by exploring more complex games with more players, actions, and information asymmetries.

Second, we only test three publicly available models. Future work should test a wider range of models, including proprietary models from government and industry, which may be fine-tuned on different data. Third, our qualitative analysis is based on a stratified sample of reasoning traces. Future work could use more advanced NLP techniques to analyze the full corpus of reasoning to identify more subtle patterns. Finally, we do not explore the impact of model temperature and other hyperparameters on strategic behavior, which remains an important avenue for future research.

\section{Conclusion}

This paper asked whether LLMs can bluff. The answer is yes, but not for the right reasons. We find that LLMs can be prompted to engage in deceptive behavior, but they do so in a way that is brittle, inconsistent, and untethered from a coherent strategic logic. The gap between their ability to mimic the language of strategy and their ability to execute a rational strategic plan is the most important finding of this paper.

This finding has profound implications for deterrence theory. Thomas Schelling famously argued that in strategic interaction, there can be a "rationality of irrationality"---a feigned madness or calculated loss of control that enhances bargaining power by manipulating shared risk. Our findings suggest that LLMs exhibit a kind of *accidental irrationality*. Their behavior is not a calculated strategy of feigned madness, but a brittle, unpredictable artifact of their training data and prompt design. This is not the rationality of irrationality, but the irrationality of non-rationality. It is not a source of strategic advantage, but a profound source of risk, as it is untethered from any coherent theory of victory or cost-benefit analysis. As we integrate AI more deeply into our national security infrastructure, understanding and managing this gap between strategic language and strategic action will be one of the most critical challenges of our time.
